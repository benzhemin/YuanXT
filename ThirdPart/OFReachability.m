...
- (id)init
{
	...
	if (self != nil)
	{
		...
		NSString* host = [[NSURL URLWithString:[[YXTSettings instance] getSetting:@"server-url"]] host];
		...
		[self updateNetworkReachabilityNow];
	}
...
-(void)updateNetworkReachabilityNow{
	...
	if(!gotFlags)
	{
		NSLog(@"Unable to update network status synchronously.");
	}
	
	[self _setReachabilityFromFlags:flags];
}
